...
import { Image, ThemeButton } from '@/common/components'
...
export function Drawer({ children }: { children: React.ReactNode }): JSX.Element {
  ...
  const { signOut, hasPermission, loggedUser } = useAuth()

  const [open, setOpen] = useState(false)

  const headerTitle = useMemo((): string => {
    if (location.pathname === '/users/profile') {
      return i18n().modules.users.pages.profile.pageTitle
    }
    if (location.pathname.startsWith('/users/form')) {
      return i18n().modules.users.pages.form.pageTitle
    }
    if (location.pathname === '/users') {
      return i18n().modules.users.pages.list.pageTitle
    }
  ...
  const actions = useMemo(() => {
    ...
    if (hasPermission({ permissionKey: ALL_PERMISSION_KEYS.users.list })) {
      filteredActions.push({
        icon: <PeopleOutlinedIcon />,
        key: '/users',
        isActive: location.pathname.startsWith('/users') && location.pathname !== '/users/profile',
        label: i18n().modules.users.pages.list.pageTitle,
        handle: () => {
    ...
    <Box sx={{ display: 'flex', flex: 1 }}>
      <AppBar position="fixed" open={open}>
        <Toolbar sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Box sx={{ display: 'flex' }}>
            <ThemeButton />
            <IconButton onClick={() => signOut()} sx={{ color: '#f2f2f2' }}>
      ...
      <DrawerContainer variant="permanent" open={open}>
        ...
        <List>
          <Tooltip title={i18n().modules.users.pages.profile.pageTitle} arrow placement="right">
            <ListItem disablePadding sx={{ display: 'block' }}>
              <ListItemButton
                data-testid={`Drawer-profile-${location.pathname === '/users/profile' ? 'active' : 'inactive'}`}
                sx={{
                  minHeight: 48,
