import { getObjectValue } from '@/common/helpers/json'
...
export function mountWhere<Model>({
  ...
  const whereMatch = !where
    ...
    : where.some((whr) =>
        Object.keys(whr).every((key) => {
          if (whr[key] !== undefined && getObjectValue(model, key) !== whr[key]) {
            return false
          }
          return true
        }),
  ...
  const whereBiggerOrEqualMatch = !whereBiggerOrEqual
    ...
    : whereBiggerOrEqual.some((whr) =>
        Object.keys(whr).every((key) => {
          const value = whr[key]
          if (value !== undefined && getObjectValue(model, key) < value) {
            return false
          }
          return true
        }),
  ...
  const whereLikeMatch = !whereLike
    ...
    : whereLike.some((whr) =>
        Object.keys(whr).every((key) => {
          if (
            whr[key] !== undefined &&
            !compareStrings(String(getObjectValue(model, key)), String(whr[key]))
          ) {
            return false
          }
          return true
  ...
  const whereInMatch = !whereIn
    ...
    : whereIn.some((whr) =>
        Object.keys(whr).every((key) => {
          ...
          if (
            whrValue !== undefined &&
            (!whrValue.length || !whrValue.includes(getObjectValue(model, key)))
          ) {
            return false
          }
          return true
